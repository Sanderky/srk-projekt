	...
	return (
		<div className={styles.helloScreen}>
			...
			<div className={styles.goNextWrapper}>
				<button className={styles.goNextBtn} onClick={() => scrollDown()}>
					Przejdź dalej
				</button>
			</div>
		</div>
	...
	const renderIcon = () => {
		...
		return (
			<img
				...
				style={{ transform: expandedView ? 'rotateZ(0deg)' : 'rotateZ(-180deg)', transition: '0.5s' }}
				alt=""
			/>
		);
	};
		...
		<div style={{ ...style }}>
			...
			>
				<div className={styles.expandInfoWrapper}>
					<div className={styles.expandNumber} style={{ justifyContent: subtitle ? 'flex-start' : 'center' }}>
						{number.toString()}
					</div>
	...
	return (
		<div className={styles.expandableWrapper}>
			...
			<ExpandableView
				...
				style={{ marginBottom: '30px' }}
			>
				{selectedDoctorId ? <DatePicker selected={selectedDate} setSelected={setSelectedDate} /> : <div>Wybierz lekarza.</div>}
			</ExpandableView>
			<ExpandableView
				expandedContentHeight={350}
				expanded={false}
				number={3}
				title={'Wybierz godzinę wizyty'}
...
const Registration = () => {
	useEffect(() => {
		document.title = 'SRK - Rejestracja';
	}, []);
